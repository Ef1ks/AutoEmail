...
def add_company(company: AddCompanyModel, db: Session = Depends(get_db), get_current_user: TokenData = Depends(get_current_user)):
    if not db.query(ListOfCompanies).filter(ListOfCompanies.email == company.email).first() or db.query(ListOfCompanies).filter(ListOfCompanies.name == company.name).first():
